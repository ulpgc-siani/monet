...
import org.monet.v3.model.PersisterHelper;
import org.monet.v3.model.SummationItemList;
...
public class FieldSummationImpl extends FieldNumberImpl implements FieldSummation, Observer {
	...
	protected void reloadValue() {
		...
		try {
			...
			this.value = PersisterHelper.load(details.getData(), SummationItemList.class);
			for (org.monet.v3.model.SummationItem item : value.Items) {
	...
	private void calculate() {
		...
		if (calculatedValue != this.totalValue.doubleValue()) {
			...
			String formatDefinition = ((SummationFieldProperty) this.fieldDefinition).getFormat();
			String format = "0.##";
