...
import org.monet.api.federation.setupservice.impl.exceptions.CantSignException;
...
public class LibraryRestfull {
	...
	public static String execute(String url, HashMap<String, ContentBody> parameters, String certificateFilename, String certificatePassword) throws Exception {
		...
		try {
			String signature = LibrarySigner.signText(hash, certificateFilename, certificatePassword);
			entityBuilder.addPart("signature", new StringBody(signature, ContentType.TEXT_PLAIN));
		} catch (Exception exception) {
			throw new CantSignException(String.format("Could not sign message: %s with certificate: %s", hash, certificateFilename));
		}
