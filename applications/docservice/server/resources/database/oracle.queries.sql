...
Insert_Document=INSERT INTO ds$documents (id, id_template, state, created_date) VALUES (@id,(SELECT MAX(id) FROM ds$templates WHERE ds$templates.code=@id_template),@state,@created_date)
...
Insert_Document_Data_With_Xml_Data=INSERT INTO ds$documents_data (id_document, content_type, hash, xml_data, location) VALUES (@id_document, @content_type, @hash, @xml_data, @location)
...
Insert_Template=INSERT INTO ds$templates (code, id_document_type, created_date) VALUES (@code, @id_document_type, @created_date);id
...
Clean_Document_Preview_Data=DELETE FROM ds$documents_preview_data WHERE created_date>trunc(sysdate-@days)
...
Insert_Work_Queue_Item=INSERT INTO ds$work_queue (id_document, operation, state, extra_data) VALUES (@id_document, @operation, @state, @extra_data);id
...
Select_Document_Estimated_Time=SELECT AVG(to_milliseconds(finish_date - start_date)*(SELECT COUNT(*) FROM ds$work_queue WHERE (state=0 OR state=1 OR state=2) AND queue_date <= (SELECT MAX(queue_date) FROM ds$work_queue WHERE id_document=@id_document))) AS time FROM ds$work_queue WHERE state=3 AND rownum<=10
Reset_Work_Queue_Items_In_Progress=UPDATE ds$work_queue SET state=0 WHERE state=2 OR state=1
Select_Not_Started_Work_Queue_Items=SELECT id, id_document, operation, queue_date, state FROM ds$work_queue WHERE state=0 AND queue_date IN (SELECT MIN(queue_date) FROM ds$work_queue tmp GROUP BY queue_date, id_document HAVING ds$work_queue.id_document=tmp.id_document) AND id_document NOT IN (SELECT DISTINCT id_document FROM ds$work_queue WHERE state IN (1,2))
...
Update_Work_Queue_Item_State_To_Finish=UPDATE ds$work_queue SET state=@state, finish_date=(SELECT SYSTIMESTAMP FROM dual) WHERE id=@id
Update_Work_Queue_Item_State_To_Pending=UPDATE ds$work_queue SET state=@state WHERE id=@id
Update_Work_Queue_Item_State_To_In_Progress=UPDATE ds$work_queue SET state=@state, start_date=(SELECT SYSTIMESTAMP FROM dual) WHERE id=@id
...
Overwrite_Document_Data=UPDATE ds$documents_data d SET (d.content_type, d.hash, d.xml_data) = (SELECT s.content_type, s.hash, s.xml_data FROM ds$documents_data s WHERE s.id_document=@source_id) WHERE d.id_document=@destination_id