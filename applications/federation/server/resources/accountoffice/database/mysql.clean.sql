...
BEGIN
  ...
  DECLARE ListViews CURSOR FOR SELECT `TABLE_NAME` FROM `INFORMATION_SCHEMA`.`TABLES` WHERE `TABLE_SCHEMA` = databasename AND `TABLE_TYPE` = 'VIEW' AND (table_name LIKE 'fs$%');
  DECLARE ListTables CURSOR FOR SELECT `TABLE_NAME` FROM `INFORMATION_SCHEMA`.`TABLES` WHERE `TABLE_SCHEMA` = databasename AND `TABLE_TYPE` = 'BASE TABLE' AND (table_name LIKE 'fs$%');
