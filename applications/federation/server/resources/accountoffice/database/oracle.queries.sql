...
CREATE_FEDERATION=INSERT INTO fs$federations (name, uri, label, trusted) VALUES (@name, @uri, @label, @trusted);id
...
CREATE_BUSINESS_UNIT=INSERT INTO fs$business_units (id_federation, name, label, type, uri, secret, enable, visible) VALUES (@id_federation, @name, @label, @type, @uri, @secret, @enable, @visible);id
...
SEARCH_USERS=SELECT id, username, fullname, email, lang, human, "mode" FROM (SELECT rownum rnum, a.* FROM (SELECT * FROM fs$users WHERE (CONTAINS(username, @condition,10) > 0 OR fullname LIKE @condition_like) AND human=1) a WHERE rownum <= @start+@limit-1) where rnum >= @start
SEARCH_USERS_COUNT=SELECT COUNT(id) AS counter FROM fs$users WHERE (CONTAINS(username, @condition,10) > 0 OR fullname LIKE @condition_like) AND human=1
EXISTS_USER=SELECT id FROM fs$users WHERE TO_CHAR(id)=@id OR username=@id
...
AUTHDB_LOAD_USER=SELECT fs$users.id, username, fullname, email, lang, human FROM fs$users WHERE username=@username
AUTHDB_LOAD_USER_BY_EMAIL=SELECT fs$users.id, username, fullname, email, lang, human FROM fs$users WHERE email=@email
