 ...
 * they are wrapped in an instance of {@link DeadEvent} and
 * reposted.
 * 
 * <p>
...
public class Bus {
  ...
  /**
   * Creates a new Bus with the given {@code enforcer} for actions and the given
   * {@code identifier}.
   * 
   * @param enforcer
   *          Thread enforcer for register, unregister, and post actions.
  ...
  /**
   * Queue the {@code event} for dispatch during {@link #dispatchQueuedEvents()}
   * . Events are queued in-order of occurrence so they can be dispatched in the
   * same order.
   */
  ...
  protected void dispatchQueuedEvents() {
    ...
    // and out-of-order events. Instead, leave
  ...
  /**
   * Flattens a class's type hierarchy into a set of Class objects. The set will
   * include all superclasses (transitively), and all interfaces implemented by
   * these superclasses.
   * 
   * @param concreteClass
