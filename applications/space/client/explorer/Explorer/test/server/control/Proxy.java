...
public class Proxy extends HttpServlet {
	...
	private static Map<String, String> savedHeaders = new HashMap<>();
...
//              System.out.println(headerName.toLowerCase() + ": " + headerValue);
		...
		for (String headerName : savedHeaders.keySet())
			request.addHeader(headerName, savedHeaders.get(headerName).replace("Path=(.*)", ""));
		...
		for (int i=0 ; i<responseHeaders.length ; i++) {
			...
			String lowerHeader = header.getName().toLowerCase();
			resp.addHeader(header.getName(), header.getValue());

			if (queryString != null && queryString.contains("saveHeader") && lowerHeader.equals("set-cookie"))
				savedHeaders.put(header.getName(), header.getValue());
		}
//		resp.addHeader("Access-Control-Allow-Origin", "*");
//		resp.addHeader("Access-Control-Request-Methods", "POST, GET, UPDATE, OPTIONS");
