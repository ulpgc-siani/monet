...
import org.monet.space.explorer.control.dialogs.AddNodeDialog;
import org.monet.space.explorer.control.displays.NodeDisplay;
...
public class AddNodeAction extends Action<AddNodeDialog, NodeDisplay> {
    ...
    public void execute() throws IOException {
        ...
        if (!componentProvider.getComponentSecurity().canWrite(parent, getAccount()))
            throw new WriteEntityPermissionException();

        Node node = layerProvider.getNodeLayer().addNode(dialog.getCode(), parent);
        display.write(node);
    }
