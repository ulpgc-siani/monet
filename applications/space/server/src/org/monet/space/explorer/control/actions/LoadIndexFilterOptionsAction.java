...
public class LoadIndexFilterOptionsAction extends LoadIndexFilterAction<FilterOptionDisplay> {

    public void execute() throws IOException {
        String indexDefinitionCode = dialog.getIndexDefinition().getCode();
        String attributeCode = dialog.getFilter().getName();
        List<String> groupOptions = layerProvider.getNodeLayer().loadReferenceAttributeValues(getParentId(), indexDefinitionCode, attributeCode, getFilterNodes(), getFilterAttributesDefinition());

        display.writeList(toExplorerList(groupOptions));
    }

    private ExplorerList<Filter.Option> toExplorerList(List<String> groupOptions) {
        ExplorerList<Filter.Option> result = new ExplorerList<>();
        result.setTotalCount(groupOptions.size());

        for (final String groupOption : groupOptions)
