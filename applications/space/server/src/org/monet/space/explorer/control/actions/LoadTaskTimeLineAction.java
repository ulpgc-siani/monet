...
public class LoadTaskTimeLineAction extends Action<TaskDialog, Display> {
    private AgentLogger logger;
    private MimeTypes mimeTypes;

    private static final int TIME_LINE_HEIGHT = 30;
    private static final int TIME_LINE_WIDTH = 200;

    private static final int TIME_LINE_COLOR = 0xfff2f2f2;
    private static final int TIME_LINE_BORDER_COLOR = 0xff999999;
    ...
    public void execute() throws IOException {
        ...
        suggestStartDate = task.getInternalStartSuggestedDate();
        suggestEndDate = task.getInternalEndSuggestedDate();
        startDate = task.getInternalStartDate();
        endDate = task.getInternalEndDate();
        taskState = task.getState();

        display.setContentType(mimeTypes.get("png"));
        display.addHeader("Cache-Control", "private, no-store, no-cache, must-revalidate");
        display.addHeader("Pragma", "no-cache");

        try {
            createImage(TIME_LINE_WIDTH, TIME_LINE_HEIGHT, display.getOutputStream());
        } catch (Exception exception) {
            display.writeError(String.format(org.monet.space.explorer.model.Error.TASK_COULD_NOT_CREATE_PROGRESS_IMAGE, task.getId()));
    ...
    private void initPaints() {
        mTimeLinePaint = new Color(TIME_LINE_COLOR, true);
        mTimeLineStrokePaint = new Color(TIME_LINE_BORDER_COLOR, true);
