...
public class ActionPrintNodeTimeConsumption extends PrintAction {
	private NodeLayer nodeLayer;

	private static DateFormat dateFormatter = new SimpleDateFormat("dd/MM/yyyy");

	public ActionPrintNodeTimeConsumption() {
		super();
	...
	public String execute() {
		...
		PrintRender render = rendersFactory.get(node, template + ".html?mode=print", getRenderLink(), account);
		render.setParameter("dataRequest", dataRequest);
		render.setRange(rangeOf(dateAttribute, finalFromDate, finalToDate));

		return String.valueOf(render.isTimeConsumptionExcessive());
	}
