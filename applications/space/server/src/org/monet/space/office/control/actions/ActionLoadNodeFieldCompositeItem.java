...
public class ActionLoadNodeFieldCompositeItem extends Action {
	private NodeLayer nodeLayer;

	public ActionLoadNodeFieldCompositeItem() {
		super();
	...
	public String execute() {
		...
		int pos = Integer.valueOf(this.request.getParameter(Parameter.POSITION));
		Node node;
		...
		if (id == null || code == null) {
			throw new DataException(ErrorCode.INCORRECT_PARAMETERS, Actions.LOAD_NODE_FIELD_COMPOSITE_ITEM);
		}
		...
		Attribute attribute = node.getAttributeList().getFirstByCode(code);
		int current = 0;
		...
		FieldProperty fieldDefinition = ((FormDefinition) node.getDefinition()).getField(attribute.getCode());
		Field field = new Field(node, attribute, fieldDefinition);
		OfficeRender render = this.rendersFactory.get(field, "preview.html?mode=view", this.getRenderLink(), getAccount());
		render.setParameter(RenderParameter.VIEW, "compositeitem");
		field.setContent(render.getOutput());

		return field.toJson().toJSONString();
	}
