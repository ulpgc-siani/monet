...
public class ActionFocusNodeField extends Action {

	public ActionFocusNodeField() {
		super();
	}
	...
	public String execute() {
		...
		if ((instanceId == null) || (fieldPath == null)) {
			throw new DataException(ErrorCode.INCORRECT_PARAMETERS, Actions.FOCUS_NODE_FIELD);
		}
		...
		if (client != null) {
			...
			context.put("field", fieldPath);
			agentPushService.updateClientViewContext(userId, this.idSession, instanceId, client.getViewId(), context);
			agentPushService.pushToViewers(client, client.getViewId(), PushClientMessages.REFRESH_OBSERVER, observer);
		} else {
			agentPushService.pushToViewers(client, PushClient.generateViewId(Node.class, idNode), PushClientMessages.ADD_OBSERVER, observer);
		}
