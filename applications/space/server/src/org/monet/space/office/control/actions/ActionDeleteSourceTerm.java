...
public class ActionDeleteSourceTerm extends Action {
	private SourceLayer sourceLayer;

	public ActionDeleteSourceTerm() {
		super();
	...
	public String execute() {
		...
		if (code == null) {
			throw new DataException(ErrorCode.INCORRECT_PARAMETERS, Actions.DELETE_SOURCE_TERM);
		}

		Source<SourceDefinition> source = this.sourceLayer.loadSource(idSource);
		this.sourceLayer.deleteSourceTerm(source, code);

		return Language.getInstance().getMessage(MessageCode.TERM_DELETED);
	}
