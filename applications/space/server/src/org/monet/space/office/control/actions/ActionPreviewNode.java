...
public class ActionPreviewNode extends Action {
	private NodeLayer nodeLayer;
	private ComponentDocuments componentDocuments;

	public ActionPreviewNode() {
		super();
	...
	public String execute() {
		...
		if (idNode == null) {
			throw new DataException(ErrorCode.INCORRECT_PARAMETERS, Actions.PREVIEW_NODE);
		}
		...
		if ((bPreview == null) || (!bPreview)) {
			throw new SystemException(ErrorCode.PREVIEW_NOT_SUPPORTED, idNode);
		}

		try {
			oMethod = new GetMethod(this.componentDocuments.getPreviewUrl(this.getParameters()));
			iStatus = oHttpClient.executeMethod(oMethod);
			if (iStatus == HttpStatus.SC_NOT_FOUND) throw new SystemException(ErrorCode.PREVIEW_NODE, idNode, null);
			this.response.getOutputStream().write(oMethod.getResponseBody());
		} catch (HttpException oException) {
			throw new SystemException(ErrorCode.PREVIEW_NODE, idNode, oException);
