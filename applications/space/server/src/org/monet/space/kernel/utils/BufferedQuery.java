...
public class BufferedQuery extends BufferedReader {
	...
	private String readBlockData() {
		...
		try {
			...
			throw new DataException("Could not read buffer", String.valueOf(this.iBufferSize));
		}
	...
	public String readQuery() {
		...
		iPos = sBlockData.indexOf(Strings.SEMICOLON);
		while (iPos == -1) {
		...
		aQueries = sBlockData.split(Strings.SEMICOLON);
		...
		for (iPos = 0; iPos < aQueries.length; iPos++) {
			...
			if ((iPos == aQueries.length - 1) && (!sBlockData.substring(sBlockData.length() - 1, sBlockData.length()).equals(Strings.SEMICOLON))) {
