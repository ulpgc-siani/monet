...
public class LibraryField {
	...
	public static String getAttributeCode(String sKey) {
		Pattern oPattern = Pattern.compile(Configuration.getInstance().getValue(Configuration.MULTIPLE_FIELD_PATTERN));
		Matcher oMatcher = oPattern.matcher(sKey);

		if (!oMatcher.find()) {
			oPattern = Pattern.compile(Configuration.getInstance().getValue(Configuration.SIMPLE_FIELD_PATTERN));
