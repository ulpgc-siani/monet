...
geoXML3.parser = function (options) {
  ...
  google.maps.event.addListener(parserOptions.map, 'click', function () {
    parserOptions.infoWindow.close();
    if (geoXML3.onInfoWindowClosed) geoXML3.onInfoWindowClosed(parserOptions.infoWindow);
  });
...
// http://keithdevens.com/weblog/archive/2007/Jun/07/javascript.clone
  ...
  var render = function (responseXML, doc) {
    // Callback for retrieving a KML document: parse the KML and display it on
    // the map
    if (!responseXML) {
      ...
      for (pm = 0; pm < placemarkNodes.length; pm++) {
        ...
        if (placemark.Point) {
          ...
          if (!!parserOptions.createMarker) {
            ...
            // Check to see if this marker was created on a previous load of
            // this document
            var found = false;
            if (!!doc) {
        ...
        if (!!parserOptions.atPlacemarkCreated) parserOptions.atPlacemarkCreated(node, placemark);
  ...
  var createMarker = function (placemark, doc) {
    ...
    if (!parserOptions.suppressInfoWindows) {
      ...
      if (parserOptions.infoWindowCreated) parserOptions.infoWindowCreated(marker.infoWindow, placemark);

      // Infowindow-opening event handler
      marker.nodeId = placemark.nodeId;
      marker.extendedData = placemark.extendedData;

      if (parserOptions.overlappingMarkerSpiderfier != null) {
        if (parserOptions.overlappingMarkerSpiderfier.listenerSet == null) {
          parserOptions.overlappingMarkerSpiderfier.addListener('click', atOverlappingMarkerClick.bind(this, infoWindowOptions));
          parserOptions.overlappingMarkerSpiderfier.listenerSet = true;
        }
      }
      else
        google.maps.event.addListener(marker, 'click', atMarkerClick.bind(this, infoWindowOptions));
    }
    placemark.marker = marker;

    if (geoXML3.onMarkerCreated) geoXML3.onMarkerCreated(marker, placemark);

    return marker;
  };

  var atOverlappingMarkerClick = function(marker, scope, infoWindowOptions) {
    marker.title = scope.title;
    marker.extendedData = scope.extendedData;
    marker.nodeId = scope.nodeId;
    marker.infoWindow = scope.infoWindow;

    atMarkerClick(marker, scope, infoWindowOptions);
  };

  var atMarkerClick = function(marker, scope, infoWindowOptions) {
    if (geoXML3.onMarkerClick) geoXML3.onMarkerClick(marker);
    marker.infoWindow.close();
    marker.infoWindow.setOptions(infoWindowOptions);
    marker.infoWindow.open(scope.map, scope);
    if (geoXML3.onInfoWindowOpened) geoXML3.onInfoWindowOpened(marker.infoWindow, marker);
  };
...
// Create Polyline
  var createPolyline = function (placemark, doc) {
    ...
    if (!parserOptions.suppressInfoWindows) {
      ...
      if (parserOptions.infoWindowCreated) parserOptions.infoWindowCreated(p.infoWindow, placemark);
...
geoXML3.getOpacity = function (kmlColor) {
  // Extract opacity encoded in a KML color value. Returns a number between 0
  // and 1.
  if (!!kmlColor &&
      (kmlColor !== '') &&
      (kmlColor.length == 8)) {
...
// Combine two options objects: a set of default values and a set of override
// values
...
/**
 * Parses the given XML string and returns the parsed document in a DOM data
 * structure. This function will return an empty DOM node if XML parsing is not
...
geoXML3.fetchXML = function (url, callback) {
  ...
  if (!!geoXML3.fetchers.length) {
    ...
    if (!!window.XMLHttpRequest) {
      ...
      xhrFetcher.fetcher = new window.ActiveXObject('Microsoft.XMLHTTP'); // Some
      // IE
    }
  }
...
// nodeValue: Extract the text value of a DOM node, with leading and trailing
// whitespace trimmed
