...
public class ChangeServerName implements ICommand {
	...
	public ChangeServerName() {
		...
		result = new Item();
		result.setProperty("id", OperationIDs.ChangeServerName);
		caption = new Item();
	...
	private boolean initialize(Item command) {
		try {
			...
			String error = "Server id not found in parameters.";
			logger.error(error);
	...
	private boolean changeName() {
		...
		servers.getItem(server).setProperty("name", newName);
